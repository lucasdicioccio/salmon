...
systemdService systemctl t cfg =
  ...
  withBinary systemctl callSystemctl (Up cfg.config_target) $ \up ->
    op "systemd-service" (deps [configContents, run t cfg]) $ \actions -> actions {
